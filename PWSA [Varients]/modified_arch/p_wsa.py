...
class DualHeadPWSA_Loss(nn.Module):
    ...
    def forward(self, preds, targets, patches_orig, patches_recon, dwt_layer):
        ...
        return total, task_loss.item() # Return scalar task loss for tracking
...
def plot_learning_curves(train_dict, val_dict, horizon):
    ...
    epochs = range(1, len(train_dict['Learnable']) + 1)
...
def plot_final_bar_charts(final_results, horizons):
    ...
    plt.title('P-WSA Multivariate Benchmarks: MSE by Horizon')
    ...
    plt.title('P-WSA Multivariate Benchmarks: Directional Accuracy by Horizon')
    plt.xticks(x, [f'H={h}' for h in horizons])
...
HORIZONS   = [1, 12, 24,36, 48, 60, 72, 84]
BATCH_SIZE = 32
EPOCHS     = 30
PATIENCE   = 10
LR         = 5e-4 
...
print(">>> RUNNING FULL P-WSA ABLATION SUITE <<< \n")
...
for PRED_LEN in HORIZONS:
    ...
    train_ds, val_ds, test_ds = [ETTDataset(SEQ_LEN, PRED_LEN, split) for split in ['train', 'val', 'test']]
    ...
    m_abl = PatchedWSA(SEQ_LEN, PRED_LEN, D_MODEL, tau_type='learnable').to(DEVICE) # Will zero_lh during pass
    ...
    for epoch in range(1, EPOCHS+1):
        ...
        # Concurrent Batch Training Loop
        for bx, by in trl:
            bx, by = bx.to(DEVICE), by.to(DEVICE)
            B, C, L = bx.shape
            ...
            if not es_abl.early_stop:
                opt_abl.zero_grad()
                p, rec, orig, _, _ = m_abl(bx_flat, zero_lh=True) # Zero HF Branch
...
# Generate Final Paper Bar Charts
plot_final_bar_charts(final_results, HORIZONS)
print("\n>>> Full P-WSA Ablation Suite and Visualizations Complete! <<<")