...
# =============================================================================
# Description:
#   Benchmarking suite for Wavelet Multiresolution Transformer architectures.
#   Includes automated unit tests for forward-pass shape validation, perfect
#   reconstruction constraints in learnable DWTs, and CI-based multivariate
#   processing for accurate 1:1 comparisons on ETT datasets.
# =============================================================================
...
PATIENCE     = 10
LR           = 3e-4
SOTA_MSE     = {96: 0.334, 192: 0.377, 336: 0.426, 720: 0.491}
...
# ─────────────────────────────────────────────────────────────────────────────
# ARCHITECTURAL BLOCKS — SHARED & P-WSA v8
# ─────────────────────────────────────────────────────────────────────────────
...
class PWSAv8(nn.Module):
    def __init__(self, seq_len, pred_len, d_model=64, n_layers=3,
        ...
        self.mixers   = nn.ModuleList([
            PatchMixer(num_p, d_model, dropout=dropout, dp_prob=dp_prob) for _ in range(n_layers)
        ])
...
# ─────────────────────────────────────────────────────────────────────────────
# ARCHITECTURAL BLOCKS — PATCHWRAT
# ─────────────────────────────────────────────────────────────────────────────
...
class WRATBlock(nn.Module):
    def __init__(self, d_model, num_heads, tau=0.1, dropout=0.2):
        ...
        self.mlp_ll      = nn.Sequential(nn.Linear(d_model, d_model*4), nn.GELU(), nn.Dropout(dropout), nn.Linear(d_model*4, d_model))
        self.mlp_lh      = nn.Sequential(nn.Linear(d_model, d_model*4), nn.GELU(), nn.Dropout(dropout), nn.Linear(d_model*4, d_model))
...
class PatchWRAT(nn.Module):
    def __init__(self, seq_len, pred_len, d_model=32, num_heads=4, dropout=0.3):
        ...
        self.head  = nn.Sequential(nn.Flatten(1), nn.Dropout(dropout), nn.Linear(flat_dim, pred_len))
    ...
    def forward(self, x):
        ...
        preds = preds.view(B, C, self.pred_len)
        xr    = xr.view(B, C, -1)
        LL    = LL.view(B, C, -1)
        LH    = LH.view(B, C, -1)
...
# ─────────────────────────────────────────────────────────────────────────────
# PRE-FLIGHT TESTS & ASSERTIONS
# ─────────────────────────────────────────────────────────────────────────────
def run_architecture_tests():
    print("[TESTS] Running internal block validation...")
    B, C, L, D = 2, 7, 336, 64
    pred_len = 96
    
    # 1. Test Moving Avg
    mad = MovingAvgDecomp(25)
    dummy_in = torch.randn(B, C, L)
    res, tr = mad(dummy_in)
    assert res.shape == (B, C, L), "MovingAvgDecomp residual shape mismatch"
    assert tr.shape == (B, C, L), "MovingAvgDecomp trend shape mismatch"
    
    # 2. Test RevIN
    revin = RevIN()
    normed = revin(dummy_in, 'norm')
    denormed = revin(normed, 'denorm')
    assert torch.allclose(dummy_in, denormed, atol=1e-4), "RevIN denorm failed to recover input"
    
    # 3. Test PatchEmbedding
    pe = PatchEmbedding(patch_len=16, stride=8, d_model=D)
    patched = pe(dummy_in[:, 0:1, :]) # Univariate test
    assert patched.shape[1] == D, "PatchEmbedding d_model mismatch"
    
    # 4. Test Learnable DWT & Perfect Reconstruction
    dwt = LearnableDWT1D(1, D)
    ll, lh = dwt(dummy_in[:, 0:1, :])
    assert ll.shape[-1] == L // 2, "DWT length downsampling failed"
    recon = dwt.inverse(ll, lh)
    assert recon.shape[-1] == L, "IDWT length upsampling failed"
    
    # 5. Test FreqSparseAttn
    fsa = FreqSparseAttn(d_model=D, num_heads=4)
    dummy_seq = torch.randn(B, 42, D) # L=42 patches
    attn_out = fsa(dummy_seq, dummy_seq, dummy_seq)
    assert attn_out.shape == (B, 42, D), "FreqSparseAttn shape mismatch"
    
    # 6. Test Model Forward Passes (Shape compliance)
    pwsa = PWSAv8(seq_len=L, pred_len=pred_len)
    out_pwsa = pwsa(dummy_in)
    assert out_pwsa.shape == (B, C, pred_len), f"P-WSA output shape {out_pwsa.shape} != {(B, C, pred_len)}"
    
    wrat = PatchWRAT(seq_len=L, pred_len=pred_len)
    out_wrat, xr, _, _ = wrat(dummy_in)
    assert out_wrat.shape == (B, C, pred_len), f"PatchWRAT output shape {out_wrat.shape} != {(B, C, pred_len)}"
    assert xr.shape == (B, C, L), "PatchWRAT reconstruction shape mismatch"
    
    print("[TESTS] All blocks passed.\n" + "="*70)

# ─────────────────────────────────────────────────────────────────────────────
# DATA, LOSS, AND TRAINING UTILS
# ─────────────────────────────────────────────────────────────────────────────
def _splits(n):
...
class ETTDataset(Dataset):
    def __init__(self, seq_len, pred_len, split='train'):
        ...
        raw_all = df.iloc[:, 1:].values.astype(np.float32) # Always multi
        n       = len(raw_all)
        te, ve  = _splits(n)
    ...
    def __getitem__(self, i): return self.data[i : i + self.seq_len].t(), self.data[i + self.seq_len : i + self.seq_len + self.pred_len].t()
...
def evaluate(model, loader, device, model_type):
    ...
    for bx, by in loader:
        ...
        if model_type == 'wrat': preds, _, _, _ = model(bx)
        else:                    preds = model(bx)
        all_p.append(preds.cpu()); all_t.append(by.cpu())
...
def pwsa_loss(preds, targets, lam_spec=0.1):
    ...
    spec = F.l1_loss(torch.fft.rfft(preds, dim=-1).abs(), torch.fft.rfft(targets, dim=-1).abs())
...
def train_pwsa(model, loader, opt):
    ...
    for bx, by in loader:
        opt.zero_grad()
        loss, mse = pwsa_loss(model(bx.to(DEVICE)), by.to(DEVICE))
...
def wrat_loss(preds, targets, x_orig, x_recon, dwt, lam_r=1.0, lam_o=0.1):
    ...
    ortho = (dwt.h.view(dwt.h.shape[0], -1) * dwt.g.view(dwt.g.shape[0], -1)).sum().abs()
...
def run_model(name, model, train_fn, val_loader, test_loader, mtype):
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if epoch % 10 == 0 or improved:
            print(f"    [{name}] ep{epoch:>3}  train={tr_loss:.5f}  val={val_m:.5f}  {'✓' if improved else ''}")
        if es.early_stop:
...
if __name__ == '__main__':
    ...
    if not os.path.exists(CSV_PATH):
        print(f"[ERROR] {CSV_PATH} not found. Please download ETTm1.")
        sys.exit(1)

    results = {h: {} for h in HORIZONS}

    for PRED_LEN in HORIZONS:
        print(f"\n{'='*70}\n  HORIZON = {PRED_LEN}  |  seq={SEQ_LEN}\n{'='*70}")
        tr_loader, va_loader, te_loader = get_loaders(PRED_LEN)

        # ── P-WSA v8 ──
        torch.manual_seed(SEED)
        pwsa = PWSAv8(SEQ_LEN, PRED_LEN).to(DEVICE)
        print(f"\n  [P-WSA v8]  params={sum(p.numel() for p in pwsa.parameters() if p.requires_grad):,}  mode=multivariate")
        results[PRED_LEN]['pwsa'] = run_model('P-WSA v8', pwsa, train_pwsa, va_loader, te_loader, 'pwsa')

        # ── PatchWRAT ──
        torch.manual_seed(SEED)
        wrat = PatchWRAT(SEQ_LEN, PRED_LEN).to(DEVICE)
        print(f"\n  [PatchWRAT] params={sum(p.numel() for p in wrat.parameters() if p.requires_grad):,}  mode=multivariate (CI)")
        results[PRED_LEN]['wrat'] = run_model('PatchWRAT', wrat, train_wrat, va_loader, te_loader, 'wrat')
        ...
        for tag, label in [('pwsa','P-WSA v8'), ('wrat','PatchWRAT')]:
            r = results[PRED_LEN][tag]; gap = r['mse'] - SOTA_MSE.get(PRED_LEN, 0)
            print(f"  │  {label:<14} {r['mse']:>8.4f}  {r['mae']:>8.4f}  {r['dir_acc']:>7.1f}%  {'+' if gap>0 else '-'}{abs(gap):.4f}          │")
        print(f"  └{'─'*62}┘")
    ...
    p_wins, w_wins = 0, 0
    for h in HORIZONS:
        pm, wm = results[h]['pwsa']['mse'], results[h]['wrat']['mse']
        win = 'P-WSA v8' if pm < wm else 'PatchWRAT'
        if pm < wm: p_wins += 1 
        else: w_wins += 1
        print(f"  {h:>5} │ {pm:>13.4f} │ {wm:>13.4f} │ {win:>12} │ {SOTA_MSE.get(h, '—')}")
    print(f"\n  Overall wins  →  P-WSA v8: {p_wins}/4   PatchWRAT: {w_wins}/4\n{'='*70}")