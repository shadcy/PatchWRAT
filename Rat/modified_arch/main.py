...
# Upgrades: Heavy Regularization, D_MODEL=16, Internal Dropout
# ============================================================
...
class WRATBlock(nn.Module):
    def __init__(self, d_model, num_heads, sparsity_tau=0.1, dropout=0.2):
        ...
        # INJECTED DROPOUT HERE to prevent internal memorization
...
class PatchWRAT(nn.Module):
    def __init__(self, seq_len, pred_len, d_model=16, num_heads=4, tau_init=0.1, tau_type='learnable', dropout=0.4):
        ...
        # INCREASED DROPOUT TO 0.4 IN HEAD
...
# ══════════════════════════════════════════════════════════════
# SECTION 4 — CONFIG & EXECUTION LOOP
# ══════════════════════════════════════════════════════════════
...
BATCH_SIZE = 64
D_MODEL    = 16   # <--- SLAHSED FROM 64 to 16 to cure overfitting!
EPOCHS     = 30
PATIENCE   = 10
LR         = 5e-4 
...
for PRED_LEN in HORIZONS:
    ...
    # 40% Dropout passed into the model
    ...
    if PRED_LEN == HORIZONS[0]:
        ...
        print(f" Parameters: {params:,} (Massively reduced for Generalization)\n")
