#  in this code ive updated the following:

# Gemini: To prove your model is doing actual signal processing (and not just performing random linear transformations), we need to extract and analyze the learned components of the Discrete Wavelet Transform (DWT).In signal processing, a Wavelet Transform is governed by two filters:h (The Low-Pass Filter): This should naturally learn to smooth the data and extract the underlying trend.g (The High-Pass Filter): This should naturally learn to detect sharp changes, volatility, and noise.I have updated your script with a new section: Section 5 — Insights & Filter Extraction.After finishing the test set for each horizon, the script will now:Extract the internal weights of the h and g filters.Print their numerical values so you can see the exact impulse response.Extract the final learned sparsity threshold ($\tau$) to see how much noise the model decided to filter out.Generate and save a PNG plot showing both the Impulse Response and the Frequency Response (Magnitude Spectrum) of the filters, proving mathematically that h learned to be a low-pass filter and g learned to be a high-pass filter.


# ============================================================
# run_patchwrat_regularized.py  —  Standalone PatchWRAT Benchmark
# ============================================================
# Single file. No modifications needed. Just run:
#   python run_patchwrat_regularized.py
#
# Dataset : ETTm1.csv (must be in the same folder)
# Upgrades: Heavy Regularization, D_MODEL=16, Filter Insight Extraction
# ============================================================
...
# ══════════════════════════════════════════════════════════════
# SECTION 2 — THE NEW PATCH-WRAT ARCHITECTURE (HEAVILY REGULARIZED)
# ══════════════════════════════════════════════════════════════
...
def train_patchwrat(model, opt, crit, loader, tau_override=None, zero_lh=False):
    ...
    for bx, by in loader:
        ...
        preds, x_recon, _, _ = model(bx, zero_lh=zero_lh)
        loss, _ = crit(preds, by, bx, x_recon, model.dwt)
...
# ══════════════════════════════════════════════════════════════
# SECTION 4 — FILTER INSIGHT EXTRACTION (NEW)
# ══════════════════════════════════════════════════════════════

def extract_and_plot_filters(model, horizon, model_type="Learnable"):
    """
    Extracts the learned DWT filters (h and g), calculates their average
    impulse response, and plots their frequency magnitude spectrum to prove
    they learned to act as valid Low-Pass and High-Pass filters.
    """
    # Detach weights and convert to numpy. Shape is (D_MODEL, 1, 4)
    h_weights = model.dwt.h.detach().cpu().numpy()
    g_weights = model.dwt.g.detach().cpu().numpy()
    
    # Average across the D_MODEL dimension to see the global filter shape
    ...
    # 2. Frequency Response (Magnitude Spectrum) using FFT
    plt.subplot(1, 2, 2)
    # Zero-pad to 64 points for a smoother frequency curve
...
print("\nStandalone PatchWRAT testing complete.")