...
# ============================================================
# Variants : Fixed tau, Adaptive tau, Learnable tau, Ablation
# Horizons : 1, 96, 192, 336, 720
# Metrics  : MAE, MSE, RMSE, sMAPE, R2, Pearson, DirAcc%
#
# Usage:
#   python run_predict.py
#   (ETTh1.csv must be in the same folder)
#
# ── What was fixed vs the original ──────────────────────────
#  1. OUTPUT PROJECTION  — dwt.inverse() returns seq_len tokens (not
#     seq_len//2); Linear(seq_len → pred_len) head maps each horizon.
#  2. RevIN — per-sample mean/std removal before model, restored on
#     output. Eliminates non-stationary drift (negative R2 at long H).
#  3. GATED LH cross-attention — soft gate [0.1,1.0] instead of hard
#     -inf mask; prevents softmax NaN on the LH branch.
#  4. LOSS REBALANCING — lambda_recon 1.0→0.1; task loss dominates.
#  5. DIRECTIONAL LOSS — soft sign loss added (lambda_dir=0.05) so the
#     model explicitly trains for direction accuracy, not just MSE.
#  6. EARLY STOPPING — patience=15, monitors val MAE (smoother than
#     MSE); best checkpoint restored before test evaluation.
#  7. LR WARMUP + COSINE — 5-epoch linear warmup then cosine decay;
#     prevents early stopping from firing during the warmup plateau.
#  Arch unchanged: LearnableDWT1D, FrequencySparseAttention, WRATBlock,
#  WaveletTransformerLoss, LearnableTauWRATBlock all preserved.
# ============================================================
...
class LearnableDWT1D(nn.Module):
    """Learnable wavelet decomposition/reconstruction. Unchanged."""
...
class FrequencySparseAttention(nn.Module):
    ...
    FIX: energy gate now soft-scales scores (multiply by gate in [0,1])
    instead of hard-masking to -inf, which zeroed entire softmax rows.
    """
    ...
    def forward(self, q_x, k_x, v_x, energy_coeffs=None):
        ...
        if energy_coeffs is not None:
            energy = torch.abs(energy_coeffs).mean(dim=-1)            # (B, L)
...
class WaveletTransformerLoss(nn.Module):
    """
    Combined task + reconstruction + orthogonality + directional loss.
    lambda_recon : 0.1  (was 1.0 — task loss now dominates)
    lambda_dir   : 0.05 — soft sign loss trains for direction accuracy.
                   Uses tanh(delta*10) as a smooth sign proxy so gradients
                   flow even when the prediction direction is wrong.
    """
    ...
    def forward(self, preds, targets, x_orig, x_recon, dwt_layer):
        ...
        # Soft directional loss: penalise sign disagreement on consecutive steps
        if preds.shape[-1] > 1:
...
class LearnableTauWRATBlock(nn.Module):
    """WRATBlock with tau as a trainable sigmoid-bounded parameter. Unchanged."""
    def __init__(self, d_model, num_heads, tau_init=0.1):
        super().__init__()
        self.raw_tau = nn.Parameter(torch.tensor(
            math.log(tau_init / (1.0 - tau_init))))
...
class WRATModel(nn.Module):
    """
    Full WRAT model: RevIN → DWT → WRATBlock → inverse → Linear head.

    FIX: Added Linear projection head (dwt_out_len → pred_len).
    dwt.inverse() always returns SEQ_LEN/2 regardless of pred_len.
    Previously, a Conv1d(1,1,1) was applied then the result was
    truncated to pred_len via min(), giving clipped/wrong outputs.
    """
    def __init__(self, d_model, num_heads, seq_len, pred_len,
                 tau_type='fixed', tau_init=0.1):
        super().__init__()
        self.revin = RevIN()
        ...
        # dwt.inverse() reconstructs back to seq_len (stride-2 transpose conv),
        # NOT seq_len//2 — use seq_len as the projection input dimension.
    ...
    def forward(self, x, zero_lh=False):
        ...
        recon        = self.dwt.inverse(LL_o, LH_o)   # (B, 1, seq_len/2)
        out          = self.proj(recon)                # (B, 1, pred_len)
        return self.revin.denormalize(out)

    def forward_with_recon(self, x, zero_lh=False):
        """Training forward that also returns x_norm and x_recon for loss."""
        x_norm       = self.revin.normalize(x)
        LL, LH       = self.dwt(x_norm)
    ...
    @property
    def tau(self):
        return self.block.tau if hasattr(self.block, 'tau') else None


# ══════════════════════════════════════════════════════════════
# SECTION 2 — DATASET  (ETTh1 standard splits)
# ══════════════════════════════════════════════════════════════

class ETThDataset(Dataset):
    """
    ETTh1 dataset.  Standard splits:
      Train : [0,       8640)
      Val   : [8640,    11520)
      Test  : [11520,   end  )
...
EPOCHS      = 60
LR          = 1e-3
PATIENCE    = 15   # was 7 — gives models time to pass through warmup plateau
WARMUP_EP   = 5    # linear LR warmup epochs before cosine decay
TAU_FIXED   = 0.1
TAU_START   = 0.5
TAU_END     = 0.05

MODEL_NAMES = ['WRAT_Fixed', 'WRAT_Adaptive', 'WRAT_Learnable', 'WRAT_Ablation']
...
def make_model(tau_type='fixed', tau_init=TAU_FIXED, pred_len=96):
    m   = WRATModel(D_MODEL, NUM_HEADS, SEQ_LEN, pred_len,
                    tau_type=tau_type, tau_init=tau_init).to(DEVICE)
    opt = optim.AdamW(m.parameters(), lr=LR, weight_decay=1e-4)
    # Linear warmup for WARMUP_EP epochs, then cosine decay to LR*0.05
    def lr_lambda(ep):
        if ep < WARMUP_EP:
            return (ep + 1) / WARMUP_EP          # ramp 0→1
...
def train_one_epoch(model, opt, loader, tau_override=None, zero_lh=False):
    model.train()
    if tau_override is not None and hasattr(model.block, 'sparsity_tau'):
        model.block.sparsity_tau = tau_override
    total_loss = n = 0
    for bx, by in loader:
        bx, by = bx.to(DEVICE), by.to(DEVICE)
        opt.zero_grad()
        preds, x_recon, x_norm = model.forward_with_recon(bx, zero_lh=zero_lh)
...
for PRED_LEN in HORIZONS:
    ...
    m_fix, opt_fix, sch_fix = make_model('fixed',     TAU_FIXED, PRED_LEN)
    m_adp, opt_adp, sch_adp = make_model('fixed',     TAU_START, PRED_LEN)
    m_lrn, opt_lrn, sch_lrn = make_model('learnable', TAU_FIXED, PRED_LEN)
    m_abl, opt_abl, sch_abl = make_model('fixed',     TAU_FIXED, PRED_LEN)

    models_info = [
        ('WRAT_Fixed',     m_fix, opt_fix, sch_fix, False),
        ('WRAT_Adaptive',  m_adp, opt_adp, sch_adp, False),
        ('WRAT_Learnable', m_lrn, opt_lrn, sch_lrn, False),
        ('WRAT_Ablation',  m_abl, opt_abl, sch_abl, True),
    ]

    if PRED_LEN == HORIZONS[0]:
        wp = sum(p.numel() for p in m_fix.parameters())
    ...
    print(f"  {'Ep':>3}  {'Fixed':>10} {'Adaptive':>10} {'Learnable':>10} {'Ablation':>10}")
    print(f"  {'-'*48}")

    for epoch in range(1, EPOCHS+1):
        tau = get_tau(epoch, EPOCHS)

        tl_fix = train_one_epoch(m_fix, opt_fix, trl)
        tl_adp = train_one_epoch(m_adp, opt_adp, trl, tau_override=tau)
        tl_lrn = train_one_epoch(m_lrn, opt_lrn, trl)
        tl_abl = train_one_epoch(m_abl, opt_abl, trl, zero_lh=True)

        for sch in [sch_fix, sch_adp, sch_lrn, sch_abl]:
            sch.step()

        for name, tl in zip(MODEL_NAMES, [tl_fix, tl_adp, tl_lrn, tl_abl]):
            hist[name]['train'].append(tl)

        for name, m, _, _, zlh in models_info:
            if no_improve[name] >= PATIENCE:
                hist[name]['val'].append(hist[name]['val'][-1])
                continue
            v = evaluate(m, vll, DEVICE, inv_fn, zero_lh=zlh)
            monitor = v['mae']   # MAE is smoother than MSE — fewer false early stops
        ...
        if epoch % 10 == 0 or epoch == 1:
            print(f"  {epoch:>3}  "
                  ...
                  f"{hist['WRAT_Ablation']['val'][-1]:>10.5f}")
    ...
    for name, m, _, _, _ in models_info:
        if best_ckpt[name] is not None:
            m.load_state_dict(best_ckpt[name])

    hr = {}
    for name, m, _, _, zlh in models_info:
        hr[name] = evaluate(m, tel, DEVICE, inv_fn, zero_lh=zlh)

    all_results[PRED_LEN]   = hr
    all_histories[PRED_LEN] = hist

    print(f"\n  Test results — Horizon {PRED_LEN}:")
    print(f"  {'Model':<18} {'MAE':>8} {'RMSE':>8} {'sMAPE%':>8} {'R2':>7} {'DirAcc%':>9} {'StopEp':>7}")
    print(f"  {'-'*68}")
    for name, m, _, _, _ in models_info:
        r = hr[name]
        print(f"  {name:<18} {r['mae']:>8.4f} {r['rmse']:>8.4f} "
...
def summary_table(metric, label, higher):
    ...
    print(f"{'='*76}")
    header = f"  {'Model':<18}" + "".join(f"  H={h:<6}" for h in HORIZONS) + "    AVG"
    print(header)
    print(f"  {'-'*72}")
    for name in MODEL_NAMES:
        vals = [all_results[h][name][metric] for h in HORIZONS]
        avg  = np.mean(vals)
        row  = f"  {name:<18}" + "".join(f"  {v:>7.4f}" for v in vals) + f"  {avg:>7.4f}"
        print(row)
...
if m_lrn.tau is not None:
    print(f"\n  Learnable tau (H={HORIZONS[-1]}): {m_lrn.tau:.4f}")
...
COLORS = {
    ...
    'WRAT_Ablation':  '#94a3b8',
}
LINES = {
    'WRAT_Fixed': '-', 'WRAT_Adaptive': '--',
    'WRAT_Learnable': '-.', 'WRAT_Ablation': ':'
}
MKR = {'WRAT_Fixed': 'o', 'WRAT_Adaptive': 's',
       'WRAT_Learnable': '^', 'WRAT_Ablation': 'x'}

fig, axes = plt.subplots(2, 3, figsize=(16, 9))
fig.suptitle("WRAT Variants — ETTh1 Multi-Horizon Benchmark",
...
for col, (metric, label) in enumerate([
    ...
    for name in MODEL_NAMES:
        vals = [all_results[h][name][metric] for h in HORIZONS]
        lw   = 2.5 if not name.endswith('Ablation') else 1.3
...
for col, h in enumerate([96, 336, 720]):
    ...
    if h in all_histories:
        for name in MODEL_NAMES:
            ...
            lw = 2.0 if not name.endswith('Ablation') else 1.1
            ax.plot(ep_x, vals, color=COLORS[name], linewidth=lw,
...
plt.tight_layout()
plt.savefig('wrat_etth1_benchmark.png', dpi=150, bbox_inches='tight')
plt.show()
print("\nPlot saved -> wrat_etth1_benchmark.png")
print("\nAll done.")